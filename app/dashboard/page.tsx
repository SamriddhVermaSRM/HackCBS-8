...
const page = () => {
  ...
  const ChartFromLogs: React.FC<{ title?: string; logs: any[] }> = ({ title, logs }) => {
        ...
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ left: 0, right: 0 }}>
              ...
              <Tooltip />
              <Area dataKey="count" type="natural" stroke="#2563eb" fill="url(#g1)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
                    ...
                    users.map((u) => (
                      <button
                        ...
                        className={`w-full text-left px-2 py-1 rounded-md mb-1 hover:bg-gray-100 dark:hover:bg-slate-800 ${
                          selectedUser?._id === u._id ? "bg-gray-100 dark:bg-slate-800" : ""
                        }`}
                      >
            ...
            <div className="col-span-2">
              <div className="aspect-video">
                <div className="w-full h-full">
                  ...
                  {selectedUser ? (
                    <div>
                      <h3 className="text-sm font-semibold mb-2">Overall activity for {selectedUser.name || selectedUser.email}</h3>
                      {loadingLogs ? (
                ...
                {selectedUser ? (
                  <div className="space-y-3 max-h-[220px] overflow-auto p-2">
                    ...
                    {!loadingLogs && groupedModules.length === 0 && (
                      <div className="text-sm text-muted-foreground">No logs for this user</div>
                    )}
                    {groupedModules.map((m) => (
                      <div key={m.moduleId} className="p-2 bg-white/5 rounded-md">
